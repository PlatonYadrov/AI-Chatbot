...
class MifConnector:
    """
    Incremental fetch for EPUB/FB2/RTF files from MIF-like storage.
    """
    ...
    def fetch_incremental(self):
        ...
        acl = ["dept:HR", "role:manager", "geo:RU"]
        meta = {"collection": "Policies", "last_modified": watermark.isoformat()}
        yield RawDoc(
            doc_id=f"{self.connector_name}-epub-1",
            source=self.connector_name,
            url=None,
            path="/mnt/mif/policies/handbook.epub",
...
if __name__ == "__main__":
    ...
    MifConnector(since=since_dt, full=full).run()
